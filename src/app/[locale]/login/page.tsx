'use client'

import { Button } from "@/components/ui/button"
import Link from 'next/link'
import { Footer } from '@/components/shared/Footer'
import { Separator } from '@/components/ui/separator'
import { LoginForm } from './components/LoginForm'
import { loginSchema } from '@/utils/schema'
import { z } from 'zod'
import { useLocale, useTranslations } from 'next-intl'
import { useRouter } from 'next/navigation'
import { useAuthUser } from '@/hooks/useUsers'
import { toast } from '@/hooks/use-toast'
import { Toaster } from "@/components/ui/toaster"
import { useEffect } from "react"
import { AUTH_STORAGE_KEY } from "@/contexts/AuthContext"

export default function Login() {
    const t = useTranslations("login")
    const router = useRouter()
    const locale = useLocale()
    const { fetchAuthUser, loading, error } = useAuthUser();

    async function handleLoginSubmit(values: z.infer<typeof loginSchema>) {
        const response = await fetchAuthUser({
            email: values.email,
            password: values.password
        });

        // Aguardar um momento para garantir que o estado de erro seja atualizado
        setTimeout(() => {
            if(!!response){
                toast({
                    title: `${t('toast.title')}`,
                    duration: 3000,
                });
                // Salvar no local storage o usuario e email 
                router.push(`/${locale}/dashboard`);
            } else {
                // Agora o erro deve estar disponível corretamente
                if (error === 'Email ou senha incorretos') {
                    toast({
                        title: `${t('toast.error-incorrect-credentials-title')}`,
                        description: `${t('toast.error-incorrect-credentials-description')}`,
                        variant: "destructive",
                        duration: 3000
                    });
                } else {
                    toast({
                        title: `${t('toast.error-create-user-title')}`,
                        description: `${t('toast.error-create-user-description')}`,
                        variant: "destructive",
                        duration: 3000
                    });
                }
                console.error('Erro ao fazer login:', error);
            }
        }, 0);
    }

    useEffect(() => {
        const userToken = localStorage.getItem(AUTH_STORAGE_KEY);
        if(userToken) {
            toast({
                title: `${t('toast.title')}`,
                duration: 3000,
            });
            router.push(`/${locale}/dashboard`);
        }
    },[])

  return (
    <div className="min-h-screen">
        <div className="flex flex-col gap-4 items-center justify-center bg-gray-1300 md:py-10">
            <div className="w-full max-w-md p-8 space-y-6 bg-gray-900 rounded-lg shadow-xl">
                <div className="space-y-1 text-left">
                    <h1 className="text-3xl font-medium text-gray-50 font-primary">{t('title')}</h1>
                    <p className="text-gray-200 text-sm font-normal">{t("description")}</p>
                </div>
                <Separator className="bg-gray-600" />
                <h2 className='text-2xl text-gray-50'>{t("login-now")}</h2>

                <LoginForm onSubmit={handleLoginSubmit} isLoading={loading} />
                <p className="text-sm text-center text-gray-400 font-sans">
                    {t('terms-info')}{' '}
                    <Link href="#" className="text-brand-blue-500 underline">
                        {t('terms-info-link')}
                    </Link>{' '}
                        {t('terms-info-description')}{' '}
                    <Link href="#" className="text-brand-blue-500 underline">
                        {t('privacy-link')}
                    </Link>
                    .
                </p>
                {/*
                    <div className="relative">
                        <div className="absolute inset-0 flex items-center">
                            <span className="w-full border-t border-gray-600" />
                        </div>
                        <div className="relative flex justify-center text-xs uppercase">
                            <span className="bg-gray-800 px-2 text-gray-400">{t('or-divider')}</span>
                        </div>
                    </div>

                    <Button 
                        variant="outline" 
                        className="w-full bg-gray-700 text-white border-gray-600 hover:bg-gray-600"
                        disabled={loading}
                    >
                        <svg className="w-5 h-5 mr-2" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clipPath="url(#clip0_13183_10121)"><path d="M20.3081 10.2303C20.3081 9.55056 20.253 8.86711 20.1354 8.19836H10.7031V12.0492H16.1046C15.8804 13.2911 15.1602 14.3898 14.1057 15.0879V17.5866H17.3282C19.2205 15.8449 20.3081 13.2728 20.3081 10.2303Z" fill="#3F83F8"/><path d="M10.7019 20.0006C13.3989 20.0006 15.6734 19.1151 17.3306 17.5865L14.1081 15.0879C13.2115 15.6979 12.0541 16.0433 10.7056 16.0433C8.09669 16.0433 5.88468 14.2832 5.091 11.9169H1.76562V14.4927C3.46322 17.8695 6.92087 20.0006 10.7019 20.0006V20.0006Z" fill="#34A853"/><path d="M5.08857 11.9169C4.66969 10.6749 4.66969 9.33008 5.08857 8.08811V5.51233H1.76688C0.348541 8.33798 0.348541 11.667 1.76688 14.4927L5.08857 11.9169V11.9169Z" fill="#FBBC04"/><path d="M10.7019 3.95805C12.1276 3.936 13.5055 4.47247 14.538 5.45722L17.393 2.60218C15.5852 0.904587 13.1858 -0.0287217 10.7019 0.000673888C6.92087 0.000673888 3.46322 2.13185 1.76562 5.51234L5.08732 8.08813C5.87733 5.71811 8.09302 3.95805 10.7019 3.95805V3.95805Z" fill="#EA4335"/></g><defs><clipPath id="clip0_13183_10121"><rect width="20" height="20" fill="white" transform="translate(0.5)"/></clipPath></defs>
                        </svg>
                        {t('login-google-button')}
                    </Button>
                */}
            </div>

            <div className='pb-8 md:pb-2'>
                <p className="text-sm text-center text-gray-50">
                    {t("register-text")}{' '}
                    <Link href={`/${locale}/register`} className="text-brand-blue-500 hover:underline">
                        {t("register-link")}
                    </Link>
                </p>
            </div>
        </div>
        <Toaster />
        <Footer />
    </div>
  )
}