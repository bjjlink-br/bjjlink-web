...
export default function Register() {
    ...
    async function handleRegisterSubmit(values: z.infer<typeof registerSchema>) {
        ...
        if(!!response){
            localStorage.setItem('@Bjjlink-user', JSON.stringify(response.account));
            localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify({ acess_token: response.token }));
            toast({
                title: `${t('toast.title')}`,
            ...
            if (error === 'Email ou domínio já está em uso') {
                toast({
                    title: `${t('toast.error-title-email-already-in-use')}`,
                    description: `${t('toast.error-description-email-already-in-use')}`,
                    variant: "destructive",
                    duration: 3000
                });
            } else {
    ...
    <div className="min-h-screen">
        <div className="flex flex-col gap-4 items-center justify-center bg-gray-1300 md:py-10">
            <div className="w-full max-w-md p-8 space-y-6 bg-gray-900 rounded-lg shadow-xl">
                <h1 className='text-2xl text-gray-50'>{t('title')}</h1>
