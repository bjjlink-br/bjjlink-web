...
import { GalleryImage } from "@/utils/dataSections"
...
export function UploadImage({
  ...
  // Atualiza o contexto a cada mudança
  React.useEffect(() => {
  ...
  const onDrop = React.useCallback(
    (acceptedFiles: File[]) => {
      ...
      if (onImageUpload) {
        onImageUpload([
          ...uploadedFiles.filter(f => f.file).map(f => f.file!),
          ...validFiles,
        ]);
      }
    },
  ...
  const removeImage = (index: number) => {
    setUploadedFiles(prev => {
      ...
      if (!removed.isRemote && removed.file) {
        ...
        {uploadedFiles.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4">
            ...
            <div className="flex flex-col gap-1">
              <span className="font-semibold text-base font-secondary text-gray-50">
                {t("steps.profile.upload-title")}
              </span>
              <p className="text-sm font-secondary text-gray-50">
                {t("steps.profile.upload-description")}
              </p>
              <p className="text-xs text-gray-200 font-secondary">
